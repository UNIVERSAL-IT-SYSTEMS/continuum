...
trait EventLogLifecycleLeveldb extends EventLogCleanupLeveldb with BeforeAndAfterEach {
  ...
  def logProps(logId: String): Props =
    RestarterActor.props(
      EventLogLifecycleLeveldb.TestEventLog.props(logId, batching), None)
}
...
object EventLogLifecycleLeveldb {
  ...
  class TestEventLog(id: String) extends LeveldbEventLog(id, "log-test")
    with TestEventLogLifecycle.TestEventLog {

    def error: Exception = new Exception("boom") with NoStackTrace
...
object RestarterActor {
  ...
  def props(props: Props, name: Option[String]): Props =
